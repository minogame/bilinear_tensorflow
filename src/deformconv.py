...
def deformconv2d( x, scope,
	...
	with tf.variable_scope(scope):
		offsets = cl.conv2d(x, num_outputs=c * 2, kernel_size=[3, 3], stride=1,
													padding='SAME', data_format='NCHW', biases_initializer=None,
													weights_initializer=tf.truncated_normal_initializer(stddev=0.01),
													activation_fn=tf.identity, scope='OFFSET')


		# offsets: (b, 2c, h, w) -> (b*c, h, w, 2)
		offsets = tf.reshape(offsets, [-1, x_shape[2], x_shape[3], 2])

		grid = tf.meshgrid( tf.range(x_shape[2]), tf.range(x_shape[3]), indexing='ij'	)
		grid = tf.stack(grid, axis=-1)
		grid = tf.cast(grid, 'float32')
		grid = tf.expand_dims(grid, 0)
		# grid = tf.tile(grid, [x_shape[0]*x_shape[1], 1, 1, 1])
		coords = grid + offsets
		tf.add_to_collection('70f92c137c01d89c6477c5ef22411bfe', coords)
		...
		# X_offset: (b*c, h, w) -> (b, c, h, w)
		x_offset = tf.reshape(x_offset, [-1, x_shape[1], x_shape[2], x_shape[3]])

		xx = cl.conv2d(x_offset, num_outputs=num_outputs, kernel_size=kernel_size, stride=stride,

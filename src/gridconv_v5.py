...
def batch_bilinear(x, weights_w, weights_h):
	...
	# coords_w: [ batch_size, channels, WIDTH, height ]
	# coords_h: [ batch_size, channels, width, HEIGHT ]
...
def gridconv2d( x, scope,
	...
	with tf.variable_scope(scope):
		...
		if one_c:
			weights_w = tf.tile(weights_w, [1, x_shape[1], 1, 1])
			weights_h = tf.tile(weights_h, [1, x_shape[1], 1, 1])
		...
		x_w = batch_bilinear(x, weights_w)

		x_w = cl.conv2d(x_w, num_outputs=num_outputs, kernel_size=kernel_size, stride=stride,
									activation_fn=activation_fn, padding=padding,
									normalizer_fn=normalizer_fn, normalizer_params=normalizer_params,
									weights_initializer=weights_initializer, data_format='NCHW', scope='Main_W')


		x_h = batch_bilinear(x, weights_h)

		x_h = cl.conv2d(x_w, num_outputs=num_outputs, kernel_size=kernel_size, stride=stride,
									activation_fn=activation_fn, padding=padding,
									normalizer_fn=normalizer_fn, normalizer_params=normalizer_params,
									weights_initializer=weights_initializer, data_format='NCHW', scope='Main_H')

...
class Model(ModelDesc):
	...
	def _build_graph(self, inputs):
		...
		def grid_residual(name, l, increase_dim=False, first=False):
			shape = l.get_shape().as_list()
			in_channel = shape[1]
			...
			with tf.variable_scope(name) as scope:
				b1 = l if first else BNReLU(l)
				c1 = gridconv2d('conv1', b1, out_channel, stride=stride1, activation_fn=BNReLU)
				c2 = gridconv2d('conv2', c1, out_channel)
				if increase_dim:
					l = AvgPooling('pool', l, 2)
					l = tf.pad(l, [[0, 0], [in_channel // 2, in_channel // 2], [0, 0], [0, 0]])
		...
		with argscope([Conv2D, AvgPooling, BatchNorm, GlobalAvgPooling], data_format='NCHW'), \
			...
			l = grid_residual('res2.0', l, increase_dim=True)
			for k in range(1, self.n):
				l = residual('res2.{}'.format(k), l)
			# 16,c=32

			l = grid_residual('res3.0', l, increase_dim=True)
			for k in range(1, self.n):
